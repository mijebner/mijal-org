// LangToggle — ES/EN segmented switch.
function LangToggle({ compact }) {
  const { lang, setLang } = useLang();
  const seg = (code) => ({
    border: 'none', cursor: 'pointer', fontFamily: 'inherit',
    background: lang === code ? 'var(--violeta)' : 'transparent',
    color: lang === code ? 'white' : 'var(--ink-muted)',
    fontSize: compact ? '.78rem' : '.68rem', fontWeight: 700, letterSpacing: '.06em',
    padding: compact ? '.45rem .8rem' : '.32rem .55rem', borderRadius: 6,
  ...
  return (
    <div role="group" aria-label="Idioma / Language" style={{
      ...
      background: 'rgba(30,22,57,.07)',
    }}>
      <button type="button" aria-pressed={lang === 'es'} style={seg('es')} onClick={() => setLang('es')}>ES</button>
      <button type="button" aria-pressed={lang === 'en'} style={seg('en')} onClick={() => setLang('en')}>EN</button>
    </div>
  );
}
  ...
  return (
    <nav style={navStyle}>
      <a href="#inicio" style={{ display: 'flex', alignItems: 'center', textDecoration: 'none' }}
        ...
        <img src={(window.__resources && window.__resources.logoVioleta) || "assets/logo-violeta.png"} alt="Mijal Iastrebner"
      ...
      <div className="nav-links">
        {links.map(l => (
          <span key={l.id} className={'nav-link-wrap ' + (active === l.id ? 'active' : '')}>
            ...
            <a href={'#' + l.id} style={linkCss}><T es={l.es} en={l.en} /></a>
          </span>
        ))}
        <LangToggle />
      ...
      {mobileOpen && (
        <div className="nav-mobile-panel" onClick={() => setMobileOpen(false)}>
          {links.map(l => (
            <a key={l.id} href={'#' + l.id} className="nav-mobile-link"><T es={l.es} en={l.en} /></a>
          ))}
          <div style={{ display: 'flex', justifyContent: 'center', padding: '.4rem 0 .2rem' }} onClick={e => e.stopPropagation()}>
            <LangToggle compact />
          </div>
          <a href="https://calendar.app.google/f3bcinMYo1Ro5Q5V6" target="_blank" rel="noopener"
             className="nav-mobile-cta"><T es="Hablemos" en="Let's talk" /></a>
        </div>
      )}
    </nav>
  );
...
Object.assign(window, { Nav, LangToggle });
